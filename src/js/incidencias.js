/* 
===============================================================================
NOMBRE: incidencias.js  (v2 con filtros avanzados)
DESCRIPCIÓN: Listado completo de incidencias con búsqueda y filtros:
             técnico, estado, fotos, orden cronológico.
===============================================================================
*/
/* ---------- DOM ---------- */
...
const mostrarMensaje = (texto, dur = 3000) => {
  msg.textContent = texto; msg.classList.remove('fade-out');
...
async function detectarFotos() {
  const promesas = incidencias.map(async inc => {
    try {
      ...
      const data = await res.json();
      mapaFotos[inc.id] = (data.status === 'ok' && Array.isArray(data.fotos) && data.fotos.length > 0);
    } catch {
    ...
    if (mapaFotos[inc.id]) {
      try {
        ...
        if (cont && data.status === 'ok' && data.fotos.length) {
          cont.innerHTML = data.fotos.map(f =>
            `<img src="data:image/jpeg;base64,${f.foto}" alt="Foto ${inc.id}"/>`).join('');
        }
      } catch (e) {
